...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://github.gnnat.top/https://raw.githubusercontent.com/BFDZ/Shadowsocks-Rust/master/ss-rust.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -O ss-rust.sh --no-check-certificate https://github.gnnat.top/https://raw.githubusercontent.com/BFDZ/Shadowsocks-Rust/master/ss-rust.sh && chmod +x ss-rust.sh
